...
int add_skip_chunks(wtv_chunked_buffer *cb, uint32_t offset, uint32_t flag) 
{
    ...
    buffered_seek((offset*WTV_CHUNK_SIZE)-start);
...
void get_sized_buffer(wtv_chunked_buffer *cb, uint32_t size) {
	...
	if(cb->skip_chunks[cb->chunk]!=-1 && start+size>cb->skip_chunks[cb->chunk]) {
        buffered_read(cb->buffer, cb->skip_chunks[cb->chunk]-start);
        cb->filepos+=cb->skip_chunks[cb->chunk]-start;
        buffered_seek(WTV_META_CHUNK_SIZE);
        cb->filepos+=WTV_META_CHUNK_SIZE;
        buffered_read(cb->buffer+(cb->skip_chunks[cb->chunk]-start), size-(cb->skip_chunks[cb->chunk]-start));
        cb->filepos+=cb->skip_chunks[cb->chunk]-start;
        cb->chunk++;
	}
	else {
...
LLONG wtv_getmoredata(void)
{
    ...
    if(firstcall)
    {
		...
		buffered_skip((cb.skip_chunks[cb.chunk]+WTV_META_CHUNK_SIZE)-past);
    ...
    while(1)
    {
        ...
        if( !memcmp(guid, WTV_STREAM2, 16 ) )
        {
            ...
            get_sized_buffer(&cb, 0xc);
            static unsigned char stream_type[16];
            get_sized_buffer(&cb, 16);
            ...
            && (wtvmpeg2 || (!wtvmpeg2 && len==2)))
        {
            ...
            bytesread+=(int) len;
            if(pad>0)
                get_sized_buffer(&cb, pad);
            frames_since_ref_time++;
            set_fts(); 
            return bytesread;
        }
        if(len+pad>0)
            get_sized_buffer(&cb, len+pad);
